...
async def vouch(ctx, seller: discord.Member, *, message: str):
    ...
    try:
        p = f"Write a 1-sentence thank you to {ctx.author.name} for vouching for {seller.name}."
        c = ai_client.chat.completions.create(messages=[{"role":"user","content":p}], model="llama-3.1-8b-instant", temperature=0.7)
        await ctx.send(embed=discord.Embed(description=f"💬 **AI:** {c.choices[0].message.content.strip()}", color=0x4fc3f7))
    except: pass
...
async def myvouches(ctx):
    ...
    await ctx.send("⏳ **Fetching your history...** Looking up names.")
    ...
    for msg, time, s_id, g_id in history:
        # LOOKUP SELLER NAME
        try:
            seller = await bot.fetch_user(s_id)
        ...
        guild_name = guild.name if guild else "Private/Deleted Server"
        
        embed.add_field(name=f"To {seller_name} in {guild_name}", value=f"*{time}*\n> {msg}", inline=False)

    try:
        await ctx.author.send(embed=embed)
        await ctx.send("📬 Check your DMs for your full history!")
    except:
